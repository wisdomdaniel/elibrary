...
const Settings = () => {
  ...
  const initialFaculty = user?.faculty || faculties[0];
  const initialDept = user?.department || UNIBEN_FACULTIES[initialFaculty][0];
  ...
  const [profileForm, setProfileForm] = useState({
    ...
    matNo: user?.matNo || 'RUN/CSC/22/1042'
  });
  ...
  const handleFacultyChange = (e) => {
    ...
    setProfileForm(prev => ({ ...prev, faculty: fac, department: depts[0] || '' }));
  };

  const handleSaveProfile = async (e) => {
    e.preventDefault();
    try {
      const updated = await xanoService.updateProfile(user?.id || 1, profileForm);
      login({ ...user, ...updated });
          ...
          {activeTab === 'profile' && (
            <form onSubmit={handleSaveProfile} className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="flex items-center gap-6 pb-8 border-b border-slate-50 dark:border-slate-800">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900 dark:text-white">{profileForm.name}</h3>
                  <p className="text-sm text-slate-400 font-bold uppercase tracking-wider">{user?.role} Account</p>
                </div>
              </div>

              {savedSuccess && (
                <div className="p-4 bg-green-50 text-green-700 rounded-xl font-bold text-sm flex items-center gap-2">
                  <CheckCircle2 size={18} /> Profile changes saved to backend!
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                />
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Faculty</label>
                  <select
                    value={profileForm.faculty}
                    onChange={handleFacultyChange}
                    className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 text-sm font-bold text-slate-900 dark:text-slate-100 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer"
                  >
                    {faculties.map(f => (
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 text-sm font-bold text-slate-900 dark:text-slate-100 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer"
                  >
                    {(UNIBEN_FACULTIES[profileForm.faculty] || []).map(d => (
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-6 py-3 rounded-lg text-sm font-black transition-all shadow-lg shadow-primary/20 active:scale-95"
                >
                  <Save size={18} />
                  Save Changes
                </button>
